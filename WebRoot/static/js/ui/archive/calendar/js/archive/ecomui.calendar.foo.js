...
(function() {
    ...
    ecomui.SingleCalendar = ui.SingleCalendar = function(el, params) {
    	...
    	//debugger;
    	var calHtml = '<div class="date-content-sel"><table class="year-indicator"><tr>\
    			<td class="btn-cell"><div ecui="type:control;id:prev-year-' + id + '" class="prev month-nav"></div></td>\
    			<td class="text-cell"><span id="year-text-' + id + '"></span>年</td>\
    			<td class="btn-cell"><div ecui="type:control;id:next-year-' + id + '" class="next month-nav"></div></td>\
                </tr></table><div class="ec-air"></div>\
                <table class="month-indicator"><tr><td class="btn-cell"><div ecui="type:control;id:prev-month-' + id + '" class="prev month-nav"></div></td>\
                <td class="tab-cell"><div ecui="type:grid;id:month-tab-' + id + '" id="month-tab-' + id + '"></div></td>\
    			<td class="btn-cell"><div ecui="type:control;id:next-month-' + id + '" class="next month-nav"></div></td></tr></table></div>\
        ...
        */
        ...
        for (var i = 0; i < 5; i++) {
            monthTabHtml.push('<div class="ec-month-tab-item"></div>');
        }
        g("month-tab-" + id).innerHTML = monthTabHtml.join("");
    	...
    	//单日历的部件
        yearText = this._eYearText = g("year-text-" + id);
    	calendar = this._uCalendar = get("calendar-" + id); 
        monthTab = this._uMonthTab = get("month-tab-" + id);
    	prevYearBtn = this._uPrevYearBtn = get("prev-year-" + id);
    	nextYearBtn = this._uNextYearBtn = get("next-year-" + id); 
    	prevMonthBtn = this._uPrevMonthBtn = get("prev-month-" + id);
    	nextMonthBtn = this._uNextMonthBtn = get("next-month-" + id); 

    	//绑定事件
    	calendar.ondateclick = SINGLE_CALENDAR_CLASS.ondateclick;
        prevMonthBtn.onclick = nextMonthBtn.onclick = SINGLE_CALENDAR_CLASS.onmonthmoveclick;
        prevYearBtn.onclick = nextYearBtn.onclick = SINGLE_CALENDAR_CLASS.onyearmoveclick;
    	calendar.onchange = SINGLE_CALENDAR_CLASS.oncalchange;
        monthTab.onclick = SINGLE_CALENDAR_CLASS.onmonthtabclick;
     ...
     * 设置月份tab的当前月
     */
    SINGLE_CALENDAR_CLASS.setMonthTab = function (month) {
       var monthTab = this._uMonthTab,
           items = monthTab._aItem,
           i = (month - 2 + 12) % 12,
           j = 0,
           o; 
       ...
       for (; o = items[j++];) {
            dom.setText(o.getBase(), (i % 12 == 0 ? 12 : i % 12) + "月");
            o._nMonth = i;
            o.alterClass("cur", month != i); 
            i++;
       }
    };
     ...
     * 移动月份按钮点击
     */
    SINGLE_CALENDAR_CLASS.onmonthmoveclick = function() {
       var wrapper = this.getParent(),
           cal = wrapper._uCalendar,
           prevMonthBtn = wrapper._uPrevMonthBtn; 
        
       cal.move(this == prevMonthBtn ? -1 : 1);
       cal.change();
    };
     ...
     * 移动年份点击
     */
    SINGLE_CALENDAR_CLASS.onyearmoveclick = function() {
       var wrapper = this.getParent(),
           cal = wrapper._uCalendar,
           prevYearBtn = wrapper._uPrevYearBtn; 

       cal.move(this == wrapper._uPrevYearBtn ? -12 : 12);
       cal.change();
    };

    SINGLE_CALENDAR_CLASS.onmonthtabclick = function() {
        var wrapper = this.getParent().getParent(),
            cal = wrapper._uCalendar,
            year = cal.getYear();

        cal.setDate(year, this._nMonth);
        cal.change();
    }
    ...
    */
    ...
    SINGLE_CALENDAR_CLASS.selectDate = function(year, month, date) {
        ...
        this._eYearText.innerHTML = year;
        this.setMonthTab(month);
    ...
    SINGLE_CALENDAR_CLASS.oncalchange = function() {
        ...
        */
        ...
        wrapper._eYearText.innerHTML = year;
        wrapper.setMonthTab(month);
 ...
 */
	...
	CALENDAR_EDIT_CLASS.oncalblur = function() {
        ...
        // 此处等价于this.$hide(), 只是没有调用loseFocus()
    ...
    function refreshDates(cal) {
        ...
        // 刷新 
        // 修复日历输入框参数里设置range第一次显示无效的问题 fixed by cz
        beginCalendar._uCalendar.change();
        endCalendar._uCalendar.change();
    };
