...
(function () {
    var lib = baidu, 
        ...
        UI_PAGER = ecomui.Pager 
            ...
            */
            ...
            // 首页/上一页
            htmls.push(
                '<div id="ec-pager-first-' + id + '">首页</div>',       
                '<div id="ec-pager-prev-' + id + '">上一页</div>'       
            );
            ...
            for (; pageStart <= pageEnd; pageStart++) {
                htmls.push(
                    '<div class="page-num">' + pageStart + '</div>'       
                )
            }

            // 下一页/尾页
            htmls.push(
                '<div id="ec-pager-next-' + id + '">下一页</div>',       
                '<div id="ec-pager-last-' + id + '">尾页</div>'       
            );
            htmls.push('</div>');
            ...
            o.innerHTML = '<input class="ec-pager-input" type="text" style="width:25px" /><button class="ec-pager-button">转到</button>';
            ...
            this._uFirst = g("ec-pager-first-" + id).getControl();
            this._uLast = g("ec-pager-last-" + id).getControl();
            this._uPrev = g("ec-pager-prev-" + id).getControl();
            this._uNext = g("ec-pager-next-" + id).getControl();
    ...
    UI_PAGER_ITEM_CLASS.$click = function (event) {
        ...
        else if (/first/.test(base.id)) {
            setCurPage.call(parent, 1);   
        }
        else if (/last/.test(base.id)) {
            setCurPage.call(parent, -1);   
        }
        else if (/prev/.test(base.id)) {
            setCurPage.call(parent, parent._nCurPage - 1);   
        }
        else if (/next/.test(base.id)) {
            setCurPage.call(parent, parent._nCurPage + 1);   
        }
