...
(function() {
	var lib = baidu, 
        ...
        getAncestorBy = libDom.getAncestorBy,
        ...
        */
        ...
        UI_TAB_TITLES_CLASS.$setSize = blank;
        ...
        /**
         * 获得_eContent元素中包含的ecui控件
         * @public
         *
         * @param {Number} index 索引
         * @return {Array} 符合条件的控件数组
         */
        UI_TAB_TITLES_CLASS.getInternalCtrls = function (index) {
            var cntEl = this.getItems()[index]._eContent,
                wrapper = this.getParent();    

            // 自定义的query函数
            function custom (o) {
                var outer = o.getOuter();

                return getAncestorBy(outer, function(el) {
                    return el == cntEl;
                })
            }

            return core.query({parent: wrapper, custom: custom});
        };
        ...
        UI_TAB_TITLES_CLASS.$select = function(index) {
            ...
            o._eContent.style.position = "static";
            o._eContent.style.visibility = "visible";
            o._eContent.style.display = "block"

            // 第一次选中标签时刷新控件
            this._oInitFlushed = this._oInitFlushed || {};
            if (this._oInitFlushed[index] !== true) {
                for (var i = 0, o, ctrls = this.getInternalCtrls(index); 
                        o = ctrls[i++];) {
                    try {
                        o.resize();
                    }
