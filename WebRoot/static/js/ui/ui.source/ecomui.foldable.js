...
(function() {
    ...
    ECOM_FOLDABLE_CLASS.$init = function() {
        ...
        this._bInitFlushed = false;
        
        if (initFold) {
            this._uContent.getOuter().style.visibility = "visible";
            this._uContent.getOuter().style.position = "static";
        }
    ...
    function ECOM_FOLDABLE_TOGGLECONTENT(fold) {
        var control = this.getParent(),
            ...
            initFlushed = control._bInitFlushed;
        ...
        // 初始化后第一次点击
        if (initFlushed === false) {
            if (!isShow) {
                // 对内部控件进行resize
                for (var i = 0, o, ctrls = core.query({parent:content}); o = ctrls[i++];) {
                    try {
                        o.resize();
                    }
                    catch (e) {}
    ...
    ECOM_FOLDABLE_CLASS.$resize = blank;
